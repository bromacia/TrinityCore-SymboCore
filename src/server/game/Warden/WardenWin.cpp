...
void WardenWin::HandleData(ByteBuffer &buff)
{
    ...
    // TIMING_CHECK
    {
        ...
        if (result == 0x00)
        {
            ...
            check.Action  = WardenActions(sWorld->getIntConfig(CONFIG_WARDEN_CLIENT_FAIL_ACTION));
            check.Comment = "Generic speedhack";

            TC_LOG_WARN("warden", "%s failed timing check. Action: %s", _session->GetPlayerInfo().c_str(), Penalty(&check).c_str());
            return;
        }
